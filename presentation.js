function presentation() {
  ...
  for (var i = 0; i < obj.inferences.length; i += 1)
    for (var j = 0; j < obj.inferences[i].length; j += 1)
      html +=
        "<tr><td><b>Se</b> [" +
        obj.variables_input[i].name +
        "]." +
        obj.variables_input[i].setsName[j] +
        "</td><td><b>Então</b></td><td>" +
        obj.variable_output.setsName[obj.inferences[i][j]] +
        "</td></tr>";
  html += "</table>";
...
function rap() {
  for (var i = 0; i < obj.variables_input.length; i += 1) {
    ...
    document.getElementById("variables_input").innerHTML +=
      "<div id='variable_" +
      i +
      "'>" +
      v.name +
      "<table><tr><td id='variable_" +
      i +
      "_rap'></td><td id='variable_" +
      i +
      "_info'></td></div>";
    draw_variable(v, i);
  }
  var i = 1000;
  document.getElementById("variable_output").innerHTML +=
    "<div id='variable_" +
    i +
    "'>" +
    obj.variable_output.name +
    "<table><tr><td id='variable_" +
    i +
    "_rap'></td><td id='variable_" +
    i +
...
function draw_variable(v, i) {
  ...
  paper[i] = Raphael(
    document.getElementById("variable_" + i + "_rap"),
    400,
    200
  );
  ...
  for (var j = 0; j < v.sets.length; j += 1) {
    document.getElementById("variable_" + i + "_info").innerHTML +=
      v.setsName[j] + ": " + v.sets[j] + "<br>";
...
function draw_function(s, i, ratio, color) {
  ...
  draw_path(s, i, ratio, color).attr({ fill: color, opacity: 0.3 });
}
